...
function Wishlist2() {
    ...
    const [completedGiftsCount, setCompletedGiftsCount] = React.useState(0);


    useEffect(() => {
        const collectionRef = collection(db, "gifts");
        const q = query(collectionRef, where("user", "==", "Renzo"));
      ...
      useEffect(() => {
        const fetchGifts = async () => {
          ...
          const completedGifts = querySnapshot.docs.filter(doc => doc.data().completed);
          setCompletedGiftsCount(completedGifts.length);
        };
      ...
      const addGift = async (giftData) => {
        ...
        document.querySelector('[name="giftName2"]').value = "";
        document.querySelector('[name="giftStore2"]').value = "";
        document.querySelector('[name="giftPrice2"]').value = "";
        document.querySelector('[name="giftPresentation2"]').value = "";
      };
    ...
    const handleAddGift = async (event) => {
        ...
        const giftData = {
          name: document.querySelector('[name="giftName2"]').value.trim(),
          store: document.querySelector('[name="giftStore2"]').value.trim(),
          price: document.querySelector('[name="giftPrice2"]').value.trim(),
          presentation: document.querySelector('[name="giftPresentation2"]').value.trim(),
        ...
        if (!querySnapshot.empty) {
          ...
          setGifts((prevGifts) =>
            prevGifts.map((gift) => (gift.createdAt === createdAt ? updatedGift : gift))
                ...
                // Update completedGiftsCount based on the completion status
        setCompletedGiftsCount((prevCount) =>
        updatedGift.completed ? prevCount + 1 : prevCount - 1
        );
      ...
      return(
        <div className={style.container}>
            <h2 className={style.owner}>Renzo's Wishlist</h2>
            <div className={style.title} >
                <p className={style.slothcredits}>You have {5 - gifts.length + completedGiftsCount} CrediSloths</p>
            ...
            <form className={style.form}>
                <input type="text" name="giftName2" className={style.input} placeholder="What do you want to get?" />
                <input type="text" name="giftStore2" className={style.input} placeholder="Store?" />
                <input type="text" name="giftPrice2" className={style.input} placeholder="Price?" />
                <input type="text" name="giftPresentation2" className={style.input} placeholder="Presentation?" />
